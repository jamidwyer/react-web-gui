...
import { SHOW_ALL, SHOW_ACTIVE, SHOW_COMPLETED } from '../constants/itemFilters'
...
describe('components', () => {
  describe('Footer', () => {
    ...
    it('should render filters', () => {
      const itemFilters = [SHOW_ALL, SHOW_ACTIVE, SHOW_COMPLETED]
      ...
      filters.props.children.forEach(function checkFilter(filter, i) {
        ...
        expect(a.props.filter).toBe(itemFilters[i])
        expect(a.props.children).toBe(filterTitles[i])
      })
    })

    it('shouldnt show clear button when no completed items', () => {
      const { output } = setup({ completedCount: 0 })
      const [ , , clear ] = output.props.children
      expect(clear).toBe(false)
    })

    it('should render clear button when completed items', () => {
